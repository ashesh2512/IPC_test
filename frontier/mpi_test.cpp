...
int main(int argc, char** argv) {
	...
	int print_interval = std::max(1, Nmpi / 10000);

	// Random number generator only on rank 0
	std::random_device rd;
	std::mt19937 gen(rd());
	std::uniform_int_distribution<> dist(10240, 102400);
    ...
    hipDeviceSynchronize();
      ...
      if (std::abs(h_temp[i] - expected) > 1e-3f) {
        std::cerr << "Rank " << rank << ": Validation failed at iteration "
                  << iter << ", index " << i
                  << ". Expected " << expected
                  << ", got " << h_temp[i] << std::endl;
        MPI_Abort(MPI_COMM_WORLD, -1);
      }
		}
    ...
    hipFree(d_recvbuf);
      ...
      std::cout << "[Progress] Iteration " << iter + 1 << "/" << Nmpi
                << " passed with N = " << N << std::endl;
		}
	}
